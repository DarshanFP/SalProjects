    ...
    <div class="card-body">
        <table class="table table-bordered">
            ...
            <tbody>
                @foreach($objective->activities as $activityIndex => $activity)
                <tr class="activity-timeframe-row">
                    <td class="activity-description-text">{{ $activity->activity }}</td>
                    @for($month = 1; $month <= 12; $month++)
                    <td>
                        <input type="checkbox" class="month-checkbox" name="objectives[{{ $objectiveIndex }}][activities][{{ $activityIndex }}][timeframe][months][{{ $month }}]" {{ in_array($month, $activity->timeframes->pluck('month')->toArray()) ? 'checked' : '' }}>
                    </td>
                    @endfor
                    <td><button type="button" class="btn btn-danger btn-sm remove-timeframe-row">Remove</button></td>
                </tr>
                @endforeach
            </tbody>
        </table>

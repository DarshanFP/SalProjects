                        ...
                        <tbody class="budget-rows">
                            @foreach($phase['budget'] ?? [[]] as $rowIndex => $row)
                                <tr>
                                    ...
                                    <td><input type="number" name="phases[{{ $phaseIndex }}][budget][{{ $rowIndex }}][rate_quantity]" class="form-control select-input" value="{{ old("phases.$phaseIndex.budget.$rowIndex.rate_quantity", $row['rate_quantity'] ?? '') }}" oninput="calculateRowTotal(this)"></td>
                                    <td><input type="number" name="phases[{{ $phaseIndex }}][budget][{{ $rowIndex }}][rate_multiplier]" class="form-control select-input" value="{{ old("phases.$phaseIndex.budget.$rowIndex.rate_multiplier", $row['rate_multiplier'] ?? 1) }}" oninput="calculateRowTotal(this)"></td>
                                    <td><input type="number" name="phases[{{ $phaseIndex }}][budget][{{ $rowIndex }}][rate_duration]" class="form-control select-input" value="{{ old("phases.$phaseIndex.budget.$rowIndex.rate_duration", $row['rate_duration'] ?? 1) }}" oninput="calculateRowTotal(this)"></td>

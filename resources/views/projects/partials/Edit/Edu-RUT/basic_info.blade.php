<div class="mb-4 card">
    ...
    <div class="card-body">
        ...
        <div class="mb-3">
            ...
            <textarea name="project_location" id="project_location" class="form-control" rows="3">{{ old('project_location', $basicInfo->project_location) }}</textarea>
        </div>

        <!-- Sisters' Work -->
        <div class="mb-3">
            <label for="sisters_work" class="form-label">Work of Sisters of St. Ann’s in the project area over the years</label>
            <textarea name="sisters_work" id="sisters_work" class="form-control" rows="3">{{ old('sisters_work', $basicInfo->sisters_work) }}</textarea>
        </div>

        <!-- Conditions -->
        <div class="mb-3">
            <label for="conditions" class="form-label">Prevailing Socio, Economic, and Cultural Conditions of the Beneficiaries</label>
            <textarea name="conditions" id="conditions" class="form-control" rows="3">{{ old('conditions', $basicInfo->conditions) }}</textarea>
        </div>

        <!-- Problems -->
        <div class="mb-3">
            <label for="problems" class="form-label">Problems Identified and Their Consequences</label>
            <textarea name="problems" id="problems" class="form-control" rows="3">{{ old('problems', $basicInfo->problems) }}</textarea>
        </div>

        <!-- Need -->
        <div class="mb-3">
            <label for="need" class="form-label">Need of the Project</label>
            <textarea name="need" id="need" class="form-control" rows="3">{{ old('need', $basicInfo->need) }}</textarea>
        </div>

        <!-- Criteria -->
        <div class="mb-3">
            <label for="criteria" class="form-label">Criteria for Selecting the Target Group</label>
            <textarea name="criteria" id="criteria" class="form-control" rows="3">{{ old('criteria', $basicInfo->criteria) }}</textarea>
        </div>
    </div>
</div>

...
<div class="mb-3 card">
    ...
    <div class="card-body">
        ...
        <div class="mb-3">
            ...
            <input type="number" name="amount_in_hand" class="form-control readonly-input" value="{{ old('amount_in_hand', $amountSanctioned ?? 0.00) }}" readonly>
        </div>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // Ensure event listeners are attached to all expense inputs
    const expenseInputs = document.querySelectorAll('[name="expenses_this_month[]"]');
    ...
    // Also listen to table input changes
    const table = document.querySelector('.table tbody');
    if (table) {
        table.addEventListener('input', function(event) {
            if (event.target.name === 'expenses_this_month[]') {
...
function addAccountRow() {
    ...
    newRow.innerHTML = `
        ...
        <td><input type="number" name="expenses_last_month[]" class="form-control" value="0" oninput="calculateRowTotals(this.closest('tr'))" readonly></td>
...
/**
 * Reindex all account table rows after add/remove operations
 * Updates the "No." column with sequential numbers (1, 2, 3, ...)
 * Ensures index numbers remain correct when rows are added or removed
 */
/**
...
function updateBudgetSummaryCards() {
    ...
    // In create mode, current report expenses are unapproved (report not yet submitted)
    // Approved = project approved expenses from other reports
    // Unapproved = current report expenses
    const approvedExpenses = projectApprovedExpenses;
    const unapprovedExpenses = currentReportExpenses;
    ...
    const approvedPercent = 0; // Always 0 in create mode

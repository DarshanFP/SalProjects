        ...
        <div class="card-body">
            @foreach($report->objectives as $objective)
                <div class="mb-3 objective-card">
                    ...
                    <div class="info-grid">
                        <div class="info-label"><strong>Objective: {{ $loop->iteration }}</strong></div>
                        <div class="info-value">{{ $objective->objective }}</div>
                        <div class="info-label"><strong>Expected Outcome: {{ $loop->iteration }}</strong></div>
                        <div class="info-value">{{ $objective->expected_outcome }}</div>
                        <div class="info-label"><strong>What Did Not Happen: {{ $loop->iteration }}</strong></div>
                        <div class="info-value">{{ $objective->not_happened }}</div>
                        <div class="info-label"><strong>Why Some Activities Could Not Be Undertaken: {{ $loop->iteration }}</strong></div>
                        ...
                        @if($objective->changes)
                            <div class="info-label"><strong>Why Changes Were Needed: {{ $loop->iteration }}</strong></div>
                            <div class="info-value">{{ $objective->why_changes }}</div>
                        @endif
                        <div class="info-label"><strong>Lessons Learnt: {{ $loop->iteration }}</strong></div>
                        <div class="info-value">{{ $objective->lessons_learnt }}</div>
                        <div class="info-label"><strong>What Will Be Done Differently: {{ $loop->iteration }}</strong></div>
                        <div class="info-value">{{ $objective->todo_lessons_learnt }}</div>
                    </div>

                    <div class="mb-3">
                        <h6>Activities {{ $loop->iteration }} </h6>
                        <div class="info-grid">
                            @foreach($objective->activities as $activity)
                                <div class="info-label"><strong>Month:</strong></div>
                                <div class="info-value">    {{ \Carbon\Carbon::createFromFormat('F', $activity->month)->format('F') }}</div>
                                <div class="info-label"><strong>Summary of Activities: {{ $loop->iteration }}</strong></div>
                                <div class="info-value">{{ $activity->summary_activities }}</div>
                                <div class="info-label"><strong>Qualitative & Quantitative Data: {{ $loop->iteration }}</strong></div>
                                <div class="info-value">{{ $activity->qualitative_quantitative_data }}</div>
                                <div class="info-label"><strong>Intermediate Outcomes: {{ $loop->iteration }}</strong></div>

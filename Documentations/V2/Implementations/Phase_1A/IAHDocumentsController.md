# Phase 1A — IAHDocumentsController
...
## Controller
- **Class**: `App\Http\Controllers\Projects\IAH\IAHDocumentsController`
...
- Both methods delegate to `ProjectIAHDocuments::handleDocuments($request, $projectId)` which requires the full `$request` for file handling
...
## Files Modified
- `app/Http/Controllers/Projects/IAH/IAHDocumentsController.php`
...
- [ ] Behavioral: IAH document upload (store/update) works
- [ ] Log: no new errors for IAH project type

## Notes / Risks
- Minimal refactor: removed dead code only. Attachment logic lives in `ProjectIAHDocuments::handleDocuments()`.

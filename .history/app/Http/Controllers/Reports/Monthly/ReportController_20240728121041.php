...
class ReportController extends Controller
{
    ...
    {
        Log::info('Fetching project details', ['project_id' => $project_id]);
        $project = Project::findOrFail($project_id);

        Log::info('Fetching last report for project', ['project_id' => $project_id]);
        $lastReport = DPReport::where('project_id', $project_id)->latest()->first();

        $lastExpenses = $lastReport ? DPAccountDetail::where('report_id', $lastReport->report_id)
            ->pluck('total_expenses', 'particulars')->toArray() : [];

        Log::info('Fetched last expenses', ['lastExpenses' => $lastExpenses]);

        return view('reports.monthly.create', compact('project', 'lastExpenses'));
    }
    ...
    {
        Log::info('Fetching report details', ['report_id' => $report_id]);
        $report = DPReport::with(['objectives.activities', 'accountDetails', 'photos', 'outlooks'])->findOrFail($report_id);

        Log::info('Fetching project details', ['project_id' => $report->project_id]);
    ...
    {
        Log::info('Fetching report details for review', ['report_id' => $report_id]);
    ...
    {
        Log::info('Reverting report', ['report_id' => $report_id]);
        $report = DPReport::findOrFail($report_id);

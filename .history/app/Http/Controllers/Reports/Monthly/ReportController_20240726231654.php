...
class ReportController extends Controller
{
    public function create($project_id)
    {
        try {
            ...
            Log::error('Error in creating report form: ' . $e->getMessage());
            return redirect()->back()->withErrors(['error' => 'Failed to load the report form. Please try again.']);
        }
    }
    ...
    public function store(Request $request)
{
    try {
        ...
        $validatedData['reporting_period_from'] = date('Y-m-d', strtotime($request->report_year . '-' . $request->report_month . '-01'));

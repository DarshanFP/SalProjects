...
class ReportController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        try {
            // Adding extra log to check if project_id exists in projects table
            $projectExists = Project::where('project_id', $request->project_id)->exists();
            Log::info('Checking if project exists', ['project_id' => $request->project_id, 'exists' => $projectExists]);
            ...
            // Generate report_id as project_id + incremental number
            ...
            $incrementalValue = $lastReport ? intval(substr($lastReport->report_id, strlen($request->project_id))) + 1 : 1;
            $newReportId = $request->project_id . $incrementalValue;
            ...
            Log::info('DPReport created', ['report' => $report]);
            ...
            if ($request->has('objective')) {
                foreach ($request->objective as $index => $objective) {
                    ...
                    Log::info('DPObjective created', ['objectiveModel' => $objectiveModel]);

                    // Store Activities for each Objective
                    if ($request->has('summary_activities.' . $index)) {
                        foreach ($request->summary_activities[$index] as $activityIndex => $summary_activity) {
                            $activityModel = DPActivity::create([
                            ...
                            Log::info('DPActivity created', ['activityModel' => $activityModel]);
                        }
                    }
                }
            }
            ...
            if ($request->has('particulars')) {
                foreach ($request->particulars as $index => $particular) {
                    $accountDetailModel = DPAccountDetail::create([
                    ...
                    Log::info('DPAccountDetail created', ['accountDetailModel' => $accountDetailModel]);
                }
            }
            ...
            if ($request->hasFile('photos')) {
                foreach ($request->file('photos') as $index => $photo) {
                    ...
                    Log::info('DPPhoto created', ['photoModel' => $photoModel]);
                }
            }
            ...
            if ($request->has('date')) {
                foreach ($request->date as $index => $date) {
                    ...
                    Log::info('DPOutlook created', ['outlookModel' => $outlookModel]);
                }
            }

            Log::info('Store method completed successfully');

            return redirect()->route('monthly.report.index')->with('success', 'Report created successfully.');
        } catch (\Exception $e) {
            Log::error('Error in store method', ['error' => $e->getMessage(), 'trace' => $e->getTraceAsString()]);
            return redirect()->back()->with('error', 'There was an error creating the report. Please try again.');
        }
    }

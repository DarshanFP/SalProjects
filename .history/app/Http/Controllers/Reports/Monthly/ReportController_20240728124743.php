...
class ReportController extends Controller
{
    ...
    // {
    //     Log::info('Starting create method', ['project_id' => $project_id]);
    ...
    //     $lastReport = DPReport::where('project_id', $project_id)->orderBy('created_at', 'desc')->first();
    //     $lastExpenses = [];

    //     if ($lastReport) {
    //         Log::info('Last report found', ['lastReport' => $lastReport]);

    //         $lastExpensesDetails = DPAccountDetail::where('report_id', $lastReport->report_id)->get();
    //         foreach ($lastExpensesDetails as $expense) {
    //             $lastExpenses[$expense->particulars] = $expense->total_expenses;
    //         }
    //     }
    ...
    public function create($project_id)
{
    ...
    $project = Project::where('project_id', $project_id)->firstOrFail();
    \Log::info('Project fetched', ['project' => $project]);

    $highestPhase = ProjectBudget::where('project_id', $project->project_id)->max('phase');
    \Log::info('Highest phase fetched', ['highestPhase' => $highestPhase]);

    $budgets = ProjectBudget::where('project_id', $project->project_id)
                            ->where('phase', $highestPhase)
                            ->get();
    \Log::info('Budgets fetched', ['budgets' => $budgets]);
    ...
    if ($lastReport) {
        $lastExpenses = DPAccountDetail::where('report_id', $lastReport->report_id)
            ...
            ->toArray();
        \Log::info('Last expenses fetched', ['lastExpenses' => $lastExpenses]);
    } else {
        \Log::info('No last report found for project', ['project_id' => $project_id]);
    }
    ...
    {
        Log::info('Starting edit method', ['report_id' => $report_id]);
    ...
    {
        Log::info('Starting show method', ['report_id' => $report_id]);

        $report = DPReport::with(['objectives.activities', 'accountDetails', 'photos', 'outlooks'])->findOrFail($report_id);
        $project = Project::where('project_id', $report->project_id)->firstOrFail();

        Log::info('Rendering show view', [
            'report' => $report,
            'project' => $project
        ]);
    ...
    public function index()
    {
        ...
        Log::info('Rendering index view', ['reports' => $reports]);
    ...
    {
        Log::info('Starting review method', ['report_id' => $report_id]);

        $report = DPReport::with(['objectives', 'accountDetails', 'photos', 'outlooks'])->findOrFail($report_id);

        Log::info('Rendering review view', ['report' => $report]);
    ...
    {
        Log::info('Starting revert method', ['report_id' => $report_id]);

        $report = DPReport::findOrFail($report_id);
        $report->update(['status' => 2]);

        Log::info('Report reverted successfully', ['report_id' => $report_id]);

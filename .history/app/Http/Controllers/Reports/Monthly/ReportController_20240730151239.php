...
class ReportController extends Controller
{
    ...
    private function handleObjectives($request, $report_id)
    {
        ...
        foreach ($objectives as $index => $objective) {
            $expectedOutcome = $request->input("expected_outcome.{$index}");
            if (!is_string($objective) || !is_string($expectedOutcome)) {
                \Log::error('Expected objective and expected outcome to be strings.', ['index' => $index, 'objective' => $objective, 'expectedOutcome' => $expectedOutcome]);
                continue;
            }
    ...
    private function handleActivities($request, $objective_id, $objectiveIndex)
    {
        ...
        foreach ($months as $monthIndex => $monthActivities) {
            if (!is_array($monthActivities)) {
                \Log::error('Expected monthActivities to be an array.', ['objective_id' => $objective_id, 'monthIndex' => $monthIndex, 'data' => $monthActivities]);
                continue;
            }

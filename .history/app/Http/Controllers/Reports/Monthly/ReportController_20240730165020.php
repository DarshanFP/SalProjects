...
class ReportController extends Controller
{
    ...
    private function handleActivities($request, $objective_id, $objectiveIndex) {
        ...
        $qualitative_data = $request->input("qualitative_quantitative_data.{$objectiveIndex}", []);
        $outcomes = $request->input("intermediate_outcomes.{$objectiveIndex}", []);

        foreach ($months as $monthIndex => $monthArray) {
            // Extract the first element of the month array
            $month = $monthArray[0] ?? 'Default Month';

            // Extract activity data using correct indices
            $summary = $summary_activities[$monthIndex][0]['1'] ?? 'Default summary';
            $qualitative = $qualitative_data[$monthIndex][0]['1'] ?? 'Default qualitative data';
            $outcome = $outcomes[$monthIndex][0]['1'] ?? 'Default outcome';

            \Log::info('Preparing to create activity', [
                'Month' => $month,
                'Summary' => $summary,
            ...
            $this->createActivity($objective_id, $month, $summary, $qualitative, $outcome);
        }
    }


    private function createActivity($objective_id, $month, $summary, $qualitative, $outcome) {
        $activity_id_suffix = str_pad((DPActivity::where('objective_id', $objective_id)->count() + 1), 3, '0', STR_PAD_LEFT);

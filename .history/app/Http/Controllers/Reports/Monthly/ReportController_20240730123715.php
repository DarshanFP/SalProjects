...
/**
 * Generate a unique report ID based on project_id
 */
protected function generateReportId($project_id)
{
    $latestReport = DPReport::where('project_id', $project_id)->latest('report_id')->first();
    $number = optional($latestReport)->report_id ? intval(str_after($latestReport->report_id, $project_id . '-')) + 1 : 1;
    return $project_id . '-' . str_pad($number, 3, '0', STR_PAD_LEFT);
}

/**
 * Additional private methods like handleObjectives, handleAccountDetails, handleOutlooks, handlePhotos would contain
 * specific logic to handle those parts of the request.
 */

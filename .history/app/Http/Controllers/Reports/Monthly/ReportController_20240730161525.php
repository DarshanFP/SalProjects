...
class ReportController extends Controller
{
    ...
    private function handleActivities($request, $objective_id, $objectiveIndex) {
        // Assuming the structure is consistently nested as shown in the logs
        ...
        foreach ($months as $monthIndex => $monthArray) {
            $month = $monthArray[0];  // Extract the month name from the first element of the month array
            ...
            $this->createActivity($objective_id, $objectiveIndex, $monthIndex, 0, $month, $summary, $qualitative, $outcome);
        }
    }

    private function createActivity($objective_id, $objectiveIndex, $monthIndex, $activityIndex, $month, $summary, $qualitative, $outcome) {
        $activity_id_suffix = str_pad($activityIndex + 1, 3, '0', STR_PAD_LEFT);
        $activity_id = "{$objective_id}-{$activity_id_suffix}";

        \Log::info('Inserting activity data', [
            'Month' => $month,
            'Summary' => $summary,

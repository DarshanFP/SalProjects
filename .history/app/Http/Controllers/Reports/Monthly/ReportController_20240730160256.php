...
class ReportController extends Controller
{
    ...
    private function handleActivities($request, $objective_id, $objectiveIndex) {
        ...
        $summary_activities = $request->input("summary_activities.{$objectiveIndex}", []);
        $qualitative_quantitative_data = $request->input("qualitative_quantitative_data.{$objectiveIndex}", []);
        $intermediate_outcomes = $request->input("intermediate_outcomes.{$objectiveIndex}", []);
        ...
        foreach ($months as $monthIndex => $month) {
            // Each $month here should correspond to a single month's data.
            // Since month data is an array of months, ensure correct parsing:
            $currentMonth = $month[0];  // Assuming $month is an array with the month name as the first element.

            // Fetch corresponding activity data for the current month index:
            $currentSummary = $summary_activities[$monthIndex][0]['1'] ?? null;
            $currentQualitativeData = $qualitative_quantitative_data[$monthIndex][0]['1'] ?? null;
            $currentOutcomes = $intermediate_outcomes[$monthIndex][0]['1'] ?? null;

            $this->createActivity($request, $objective_id, $objectiveIndex, $monthIndex, 0, $currentMonth, $currentSummary, $currentQualitativeData, $currentOutcomes);
        }
    }

    private function createActivity($request, $objective_id, $objectiveIndex, $monthIndex, $activityIndex, $month, $summary, $qualitative, $outcomes) {
        ...
        try {
            DPActivity::create([
                ...
                'qualitative_quantitative_data' => $qualitative,

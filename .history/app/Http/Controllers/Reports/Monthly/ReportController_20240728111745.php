...
{
    public function fetchLatestTotalExpenses($project_id)
    {
        $latestReportId = DPReport::where('project_id', $project_id)
        ...
        $expenses = DPAccountDetail::where('report_id', $latestReportId)
                                    ->get()
                                    ->pluck('total_expenses', 'particulars')
                                    ->values() // Ensure the values are indexed numerically
                                    ->toArray();

        \Log::info('Fetched expenses for latest report', ['expenses' => $expenses]); // Add logging
    ...
    public function create($project_id)
    {
        ...
        Log::info('Project details:', ['project' => $project]);

        // Fetch the highest phase
        $highestPhase = ProjectBudget::where('project_id', $project->project_id)->max('phase');
        Log::info('Highest phase:', ['highestPhase' => $highestPhase]);

        // Fetch budgets
        $budgets = ProjectBudget::where('project_id', $project->project_id)
                                ->where('phase', $highestPhase)
                                ->get();
        Log::info('Budgets:', ['budgets' => $budgets]);

        // Fetch amount sanctioned and amount forwarded
        $amountSanctioned = $project->amount_sanctioned ?? 0;
        $amountForwarded = $project->amount_forwarded ?? 0;
        Log::info('Amount sanctioned:', ['amountSanctioned' => $amountSanctioned]);
        Log::info('Amount forwarded:', ['amountForwarded' => $amountForwarded]);

        // Fetch expenses up to last month
        $expensesUpToLastMonth = $this->fetchLatestTotalExpenses($project->project_id);
        Log::info('Expenses up to last month:', ['expensesUpToLastMonth' => $expensesUpToLastMonth]);
        ...
        Log::info('New report ID:', ['newReportId' => $newReportId]);

        // Pass data to the view
        return view('reports.monthly.ReportCommonForm', compact(
            'project', 'user', 'amountSanctioned', 'amountForwarded', 'budgets', 'expensesUpToLastMonth', 'newReportId'
        ));
    }



    public function testFetchLatestTotalExpenses($project_id)
{
    $expenses = $this->fetchLatestTotalExpenses($project_id);
    ...
    public function store(Request $request)
{
    ...
    try {
        ...
        foreach ($request->particulars as $index => $particular) {
            $dpAccountDetail = DPAccountDetail::create([
                ...
                'report_id' => $dpReport->report_id,
                'project_id' => $request->project_id, // Add project_id here
    ...
    public function edit($report_id)
    {
        ...
        return view('reports.monthly.edit', compact('report', 'project', 'budgets', 'user'));
    }
        ...
        try {
            ...
            foreach ($request->particulars as $index => $particular) {
                $dpAccountDetail = DPAccountDetail::create([
                    ...
                    'amount_sanctioned' => $request->amount_sanctioned[$index],
                    'total_amount' => $request->total_amount[$index],
                    'expenses_last_month' => $this->fetchLatestTotalExpenses($request->project_id), // Using the latest total expenses here
                    'expenses_this_month' => $request->expenses_this_month[$index],
                    'total_expenses' => $request->total_expenses[$index],
                    'balance_amount' => $request->balance_amount[$index],
                ]);

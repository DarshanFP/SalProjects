...
class ReportController extends Controller
{
    public function create($project_id)
    {
        ...
        Log::info('New report ID generated', ['newReportId' => $newReportId]);
    ...
    public function store(Request $request)
    {
        ...
        try {
            ...
            $validatedData = $request->validate([
                ...
                'amount_sanctioned_overview' => 'nullable|numeric|default:0.00',
                'amount_forwarded_overview' => 'nullable|numeric|default:0.00',
                'amount_in_hand' => 'nullable|numeric|default:0.00',
                'total_balance_forwarded' => 'nullable|numeric|default:0.00',
                ...
                'amount_forwarded.*' => 'nullable|numeric|default:0.00',
                'amount_sanctioned.*' => 'nullable|numeric|default:0.00',
                'total_amount.*' => 'nullable|numeric|default:0.00',
                'expenses_last_month.*' => 'nullable|numeric|default:0.00',
                'expenses_this_month.*' => 'nullable|numeric|default:0.00',
                'total_expenses.*' => 'nullable|numeric|default:0.00',
                'balance_amount.*' => 'nullable|numeric|default:0.00',
            ...
            $dpReport = DPReport::create([
                ...
                'amount_sanctioned_overview' => $request->amount_sanctioned_overview ?? 0.00,
                'amount_forwarded_overview' => $request->amount_forwarded_overview ?? 0.00,
                'amount_in_hand' => $request->amount_in_hand ?? 0.00,
                'total_balance_forwarded' => $request->total_balance_forwarded ?? 0.00,
                'status' => 1,
            ]);
            ...
            foreach ($request->objective as $index => $objective) {
                ...
                $dpObjective = DPObjective::create([
                    ...
                    'changes' => isset($request->changes[$index]) && $request->changes[$index] === 'yes' ? 1 : 0,
                ...
                foreach ($request->summary_activities[$index] as $activityIndex => $activity) {
                    ...
                    $dpActivity = DPActivity::create([
                        ...
                        'summary_activities' => $activity[1] ?? null,
                        'qualitative_quantitative_data' => $request->qualitative_quantitative_data[$index][$activityIndex][1] ?? null,
                        'intermediate_outcomes' => $request->intermediate_outcomes[$index][$activityIndex][1] ?? null,
                    ]);
            ...
            foreach ($request->particulars as $index => $particular) {
                $accountDetailId = $newReportId . '-' . ($index + 1);

                $dpAccountDetail = DPAccountDetail::create([
                    'account_detail_id' => $accountDetailId,
                    'report_id' => $dpReport->report_id,
                    'particulars' => $particular ?? null,
                    'amount_forwarded' => $request->amount_forwarded[$index] ?? 0.00,
                    'amount_sanctioned' => $request->amount_sanctioned[$index] ?? 0.00,
                    'total_amount' => $request->total_amount[$index] ?? 0.00,
                    'expenses_last_month' => $request->expenses_last_month[$index] ?? 0.00,
                    'expenses_this_month' => $request->expenses_this_month[$index] ?? 0.00,
                    'total_expenses' => $request->total_expenses[$index] ?? 0.00,
                    'balance_amount' => $request->balance_amount[$index] ?? 0.00,
                ]);
    ...
    public function update(Request $request, $report_id)
    {
        ...
        try {
            ...
            foreach ($request->particulars as $index => $particular) {
                $accountDetailId = $report->report_id . '.' . ($index + 1);
                $dpAccountDetail = DPAccountDetail::create([
                    'account_detail_id' => $accountDetailId,
                    'report_id' => $report->report_id,
                    'particulars' => $particular ?? null,
            ...
            if ($request->hasFile('photos')) {
                foreach ($request->file('photos') as $index => $photo) {
                    $photoPath = $photo->store('photos', 'public');
                    $dpPhoto = DPPhoto::create([
                        'photo_id' => $report->report_id . '-' . str_pad($index + 1, 2, '0', STR_PAD_LEFT),
                        'report_id' => $report->report_id,
                        'photo_path' => $photoPath,
                        'description' => $request->photo_descriptions[$index] ?? '',
                    ]);

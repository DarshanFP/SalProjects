...
{
    public function fetchLatestTotalExpenses($report_id)
    {
        $latestTotalExpenses = DPAccountDetail::where('report_id', $report_id)
            ->orderBy('created_at', 'desc')
            ->value('total_expenses');

        return $latestTotalExpenses ?: 0.00;
    }

    public function getLatestReportId($project_id)
    {
        return DPReport::where('project_id', $project_id)
            ->orderBy('created_at', 'desc')
            ->value('report_id');
    }
    ...
    public function create($project_id)
    {
        ...
        $latestReportId = $this->getLatestReportId($project->project_id);
        $expensesUpToLastMonth = $latestReportId ? $this->fetchLatestTotalExpenses($latestReportId) : 0.00;
    ...
    public function store(Request $request)
    {
        ...
        try {
            ...
            $latestReportId = $this->getLatestReportId($request->project_id);
            $latestTotalExpenses = $latestReportId ? $this->fetchLatestTotalExpenses($latestReportId) : 0.00;
            ...
            foreach ($request->particulars as $index => $particular) {
                $dpAccountDetail = DPAccountDetail::create([
                    ...
                    'expenses_last_month' => $latestTotalExpenses, // Using the latest total expenses here

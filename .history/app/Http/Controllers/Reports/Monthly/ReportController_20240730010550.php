    ...
    public function store(Request $request)
{
    ...
    $validatedData = $request->validate([
        ...
        'objective.*' => 'required|string', // Objectives
        'expected_outcome.*' => 'required|string',
        'month.*.*' => 'required|string', // Month for each activity
        'summary_activities.*.*.*' => 'required|string', // Summary of activities
    ...
    if (empty($objectives)) {
        Log::warning('No objectives provided');
    }
    ...
    foreach ($objectives as $index => $objective) {
        ...
        $objectiveModel = DPObjective::create($objectiveData);
        Log::info('Objective created', ['objective_id' => $objectiveModel->id]);

        // Process each activity under the current objective
        $activities = $request->input("month.$index", []);
        foreach ($activities as $activityIndex => $month) {
            $activityData = [
                'objective_id' => $objectiveModel->id,
                'month' => $month,
                'summary_activities' => $request->input("summary_activities.$index.$activityIndex.1"),
                'qualitative_quantitative_data' => $request->input("qualitative_quantitative_data.$index.$activityIndex.1"),
                'intermediate_outcomes' => $request->input("intermediate_outcomes.$index.$activityIndex.1")
            ];
    ...
    // Handle photos and descriptions
    ...
    if (empty($dates)) {
        Log::warning('No outlooks provided');
    }
    ...
    foreach ($dates as $index => $date) {
        ...
        Log::info('Outlook created', ['outlook_id' => $outlook->id]);
    }

...
class ReportController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        try {
            ...
            // Handle objectives
            $this->handleObjectives($request, $report_id);
            ...
            return back()->withErrors($ve)->withInput();
        } catch (\Exception $e) {
            DB::rollBack();
            Log::error('Failed to create report', ['error' => $e->getMessage()]);
            return back()->withErrors(['msg' => 'Failed to create report due to an error: ' . $e->getMessage()])->withInput();
        }
    }

    private function handleObjectives($request, $report_id)
    {
        $objectives = $request->input('objective', []);
        if (!is_array($objectives)) {
            \Log::error('Expected objectives to be an array.', ['data' => $objectives]);
            return;
        }
        ...
        foreach ($objectives as $index => $objective) {
            ...
            try {
                ...
                \Log::error('Failed to create objective.', ['error' => $e->getMessage(), 'objective_id' => $objective_id]);
            }

            // Handle activities for each objective
            $this->handleActivities($request, $objective_id, $index);
        }
    }

    private function handleActivities($request, $objective_id, $objectiveIndex)
    {
        \Log::info('Activity Data', [
            'Summary' => $request->input("summary_activities.{$objectiveIndex}.{$monthIndex}.{$activityIndex}"),
            'Qualitative' => $request->input("qualitative_quantitative_data.{$objectiveIndex}.{$monthIndex}.{$activityIndex}"),
            'Outcomes' => $request->input("intermediate_outcomes.{$objectiveIndex}.{$monthIndex}.{$activityIndex}")
        ]);
        $months = $request->input("month.{$objectiveIndex}", []);
        if (!is_array($months)) {
            \Log::error('Expected months to be an array.', ['objective_id' => $objective_id, 'data' => $months]);
            return;
        }

        foreach ($months as $monthIndex => $monthActivities) {
            if (is_string($monthActivities)) {
                // Handle the case where monthActivities is a single string
                $this->createActivity($request, $objective_id, $objectiveIndex, $monthIndex, 0, $monthActivities);
            } else if (is_array($monthActivities)) {
                foreach ($monthActivities as $activityIndex => $month) {
                    $this->createActivity($request, $objective_id, $objectiveIndex, $monthIndex, $activityIndex, $month);
                }
            } else {
                \Log::error('Unexpected data type for monthActivities.', ['objective_id' => $objective_id, 'monthIndex' => $monthIndex, 'data' => $monthActivities]);
            }
        }
    }

    private function createActivity($request, $objective_id, $objectiveIndex, $monthIndex, $activityIndex, $month) {
        ...
        try {
            DPActivity::create([
                ...
                'objective_id' => $objective_id,
                'month' => $month,
                'summary_activities' => $request->input("summary_activities.{$objectiveIndex}.{$monthIndex}.{$activityIndex}"),
                'qualitative_quantitative_data' => $request->input("qualitative_quantitative_data.{$objectiveIndex}.{$monthIndex}.{$activityIndex}"),
                'intermediate_outcomes' => $request->input("intermediate_outcomes.{$objectiveIndex}.{$monthIndex}.{$activityIndex}")
            ]);
        } catch (\Exception $e) {
            \Log::error('Failed to create activity.', ['error' => $e->getMessage(), 'activity_id' => $activity_id]);
        }
    }
